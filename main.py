...
class CharmRenderer:
    ...
    def load_charm(self):
        try:
            asset_path = Path(__file__).resolve().parent / "assets" / "charm.png"
    ...
    def draw(self, anchor_x: float, anchor_y: float, charm_x: float, charm_y: float, angle: float):
        ...
        if self.charm_image:
            rotation_angle = -angle * 180 / math.pi * 0.3
            self.charm_id = self.canvas.create_image(
                int(charm_x), int(charm_y),
                image=self.charm_image, anchor=tk.CENTER
            )
...
class DesktopDangleApp:
    def __init__(self):
        ...
        self.anchor_x = self.config.window_width - self.config.margin_right
        self.anchor_y = 0
    ...
    def on_mouse_down(self, event):
        ...
        charm_radius = max(self.renderer.charm_width, self.renderer.charm_height) / 2
        if distance <= charm_radius or self.physics.is_point_near_rope(event.x, event.y):
